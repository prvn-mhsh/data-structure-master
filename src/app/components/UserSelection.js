...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('ds_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('ds_quizUserId', newUserId);
    ...
    localStorage.setItem('ds_quizResults', JSON.stringify(updatedUsers));
