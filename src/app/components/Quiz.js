...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('ds_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('ds_quizResults') || '[]');
      ...
      localStorage.setItem('ds_quizResults', JSON.stringify(updatedResults));
    }
  };
